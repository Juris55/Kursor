          ...
          >
            <div style={{ display: 'flex', gap: 12, height: '100%', alignItems: 'center' }} onClick={e => e.stopPropagation()}>
              ...
              <div
                ...
                onClick={() => {
                  const text = aiResultsText || activeFile?.content || ''
                  if (text) navigator.clipboard.writeText(text)
                }}
                title={t('editor.copyTranslated')}
              >
                📋 {t('editor.copy')}
              </div>
            </div>
            <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }} onClick={e => e.stopPropagation()}>
              {bottomTab === 'analysis' && aiResultsText && (
                <button
                  className="btn btn-sm"
                  style={{ fontSize: 9, padding: '1px 4px', height: 18, background: 'var(--accent)' }}
                  onClick={() => {
                    navigator.clipboard.writeText(aiResultsText)
                    alert('Rezultāti nokopēti!')
                  }}
                  title={t('editor.copyTranslated')}
                >
                  📋 {t('editor.copy')}
